...
class HighDimensionalEnhancement(nn.Module):
    def __init__(self, base_size=32):
        super(HighDimensionalEnhancement, self).__init__()
        self.base_size = base_size
        self.dropout = nn.Dropout(p=0.1)

    def forward(self, features):

        #random_number = torch.rand(1) * 0.25
        random_number = 0.25
        semantic_vector = random_number * torch.randn(features.size(1))
        semantic_vector = semantic_vector.to(features.device)
        features = features + semantic_vector
        #print("1:",features.size())# torch.Size([32, 512])

        #


        features = self.random_channel_crop(features)
        #print("2:", features.size())#torch.Size([32, 497])
        ...
        #features = self.random_channel_shuffle(features)
        #print("3:", features.size())#torch.Size([32, 497])
        # Dropout
        #features = self.dropout(features)
        #print("4:", features.size())#torch.Size([32, 497])
        return features

    def random_channel_crop(self, features):
        batch_size, num_channels = features.size(0), features.size(1)
        #crop_ratio = random.uniform(0.75, 1.0)
        crop_ratio = 0.75
        crop_channels = int(num_channels * crop_ratio)
        start_idx = random.randint(0, num_channels - crop_channels)
        end_idx = start_idx + crop_channels
        #print('1:',features)
        features[:, start_idx:end_idx] = 0
        #print('2:',features)
        #cropped_features = features[:, start_idx:end_idx]
        return features

    def random_channel_shuffle(self, features):
        batch_size, num_channels = features.size(0), features.size(1)
...
class FeatureEnhancer(nn.Module):
    ...
    def forward(self, x):
        random_number = torch.rand(1) * 0.25
        semantic_vector = random_number * torch.randn(x.size(1))
        semantic_vector = semantic_vector.to(x.device)
        enhanced_features = x + semantic_vector
...
def get_fcs(num_in, num_out, num_fcs, device=None):
    ...
    # if device:
    #     fcs = FC(num_in, num_out).to(device)
    #
    # else:
    #     fcs = FC(num_in, num_out)
    return fcs
...
def get_pretrained_model(model_name, is_train=True, device=None):
    ...
    #model = ptm.__dict__[model_name](num_classes=1000, pretrained=None)
    #model = models.resnet18(pretrained= False )
    ##########################
    enhancer = FeatureEnhancer()#####


    #model.fc = nn.Sequential(enhancer)
...
def get_kfold_img_idx(p, k, dataset, sample_type=None):  
    ...
    #print('train_line0',len(train_line))####358
    val_line = [int(str) for str in val_line.rstrip().split(' ')]
    #print('val_line0', len(val_line))###89
    if sample_type:
        if sample_type == 'uniform':
            ...
            for i in range(len(class_slices) - 1):
                #print("i",i)#####0,1  KLSG_class_slices = [0, 62, 447]
                ...
                #print(len(balance_train_line))###48-->96
            train_line = balance_train_line
        else:
            print(f'ERROR! sample_type {sample_type} is not exist!')
            return None
    #print('train_line', len(train_line))####358-->96
    #print('val_line', len(val_line))#####89
...
'''
def evaluate_accuracy1(data_iter, backbone, fc, device=None):
    ...
    with torch.no_grad():
        for X, y in data_iter:
            if isinstance(backbone, torch.nn.Module):
                backbone.eval()
                fc.eval()
                feature = backbone(X.to(device))
                acc_sum += (fc(feature).argmax(dim=1) == y.to(device)).float().sum().cpu().item()
                backbone.train()
                fc.train()
            else:
                if('is_training' in backbone.__code__.co_varnames):

                    acc_sum += (fc(backbone(X, is_training=False)).argmax(dim=1) == y).float().sum().item()
                else:

                    acc_sum += (fc(backbone(X)).argmax(dim=1) == y).float().sum().item()
            n += y.shape[0]
    return acc_sum / n
...
def train_EA(train_iter, test_iter, backbone, ea, fc, loss, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            ...
            y = y.to(device)
            feat = backbone(X)
            feat= torch.unsqueeze(feat, dim=1)
            #print('feat:',feat.size())#torch.Size([32, 512])
            G_feat = EA(feat)
            G_feat = torch.squeeze(G_feat)
            y_hat = Fc(G_feat)
            l = loss(y_hat, y)
            optimizer.zero_grad()
            l.backward()
...
def train_FE(train_iter, test_iter, backbone, FE, fc, loss, optimizer,scheduler, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            ...
            batch_count += 1
            scheduler.step()
        test_acc = evaluate_accuracy1(test_iter, backbone, fc)
        ...
        #print('epoch %d, loss %.4f, train acc %.3f, test acc %.3f, time %.1f sec'
         #     % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n, test_acc, time.time() - start))
def train1(train_iter, test_iter, backbone, FE, fc, loss, optimizer, device, num_epochs):
...
Balanced muti-branch training function.
    ...
    @ num_epochs : training epochs
'''
def train_mul(train_iter,test_iter, backbone, FE, fc, loss, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            ...
            y = y.to(device)
            features = backbone(X)
            # features0 = torch.unsqueeze(features0, dim=1)
            #features = FE(features)
            # features = torch.squeeze(features)
            #############semantic_gaussian_sampler
            ####print('semantic_vector',semantic_vector.size())####torch.Size([512])
            # random_number = torch.rand(1) * 0.25
            # semantic_vector = random_number * torch.randn(features.size(1))
            ...
            #print('features',features.size())####torch.Size([32, 512])
            y_hat = fc(features)
...
def cal_y_hat(fcs, features, method, device, fusion_net = None, if_get_logits=False):
    ...
    if method == 'normal':
        ...
        fus_fc = fcs[0]
        y_hat = fus_fc(features).argmax(dim=1)###choose the bigger valuses
        if if_get_logits:
...
def evaluate_gmean_optional(data_iter, backbone, fcs, method, fusion_net=None, device=None, if_get_y=False, if_get_logits=False):
    ...
    with torch.no_grad():
        for X, y in data_iter: 
            ...
            features = backbone(X)
            ###############################################
            # semantic_vector = torch.randn(features.size(1)).to(device)
            # features = features + semantic_vector
            #################################################
...
def ensemble_pruning(train_iter, backbone, fcs, _p=0.6, use_single_metric=None):
    ...
    if use_single_metric == 'gmean':
        return gmean_sort[0:int(len(gmean_sort)*_p)]
    elif use_single_metric == 'acc':
